 ...
 */
@WebServlet("/ncontrol")
@MultipartConfig(maxFileSize = 1024 * 1024 * 4, location = "c:/Temp/img")
public class NewsController extends HttpServlet {
	...
	public String listNews(HttpServletRequest request) {
		...
		try {
			...
			e.printStackTrace();
			ctx.log("뉴스목록 에러");
			request.setAttribute("error", "뉴스목록에러 발생");
		}
		return "newslist.jsp";
	}
		...
		try {
			...
			e.printStackTrace();
			ctx.log("뉴스가져오기 문제");
			request.setAttribute("error", "뉴스 제대로 못가져옴");
		}
		return "newsview.jsp";
	}
	...
	public String deleteNews(HttpServletRequest request) {
		...
		try {
			...
			e.printStackTrace();
			ctx.log("뉴스 삭제 문제");
			request.setAttribute("error", "뉴스 제대로 삭제못함");
